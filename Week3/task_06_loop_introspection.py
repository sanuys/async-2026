...
async def main():
    # Check identity of the currently wrapper context
    me = asyncio.current_task()
    me.set_name("Main-Coordinator")
    print(f"{ctime()} Active Execution Context Name: {me.get_name()}")
    
    # Spawn background jobs with dynamic naming for better traceability
    tasks = [asyncio.create_task(dynamic_job(i), name=f"Job-{i}") for i in range(3)]
    
    # Peek inside active evenr loop to map workloads
    ...
    await asyncio.sleep(1.1) # Flush out the jobs
